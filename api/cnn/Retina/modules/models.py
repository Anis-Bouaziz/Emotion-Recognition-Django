...
from api.cnn.Retina.modules.anchor import decode_tf, prior_box_tf
...
class ConvUnit(tf.keras.layers.Layer):
    ...
    def __init__(self, f, k, s, wd, act=None, name='ConvBN', **kwargs):
        unique_id_1 = random.randint(1,999)
        unique_id_2 = random.randint(1,999)
        super(ConvUnit, self).__init__(name=name, **kwargs)
        self.conv = Conv2D(filters=f, kernel_size=k, strides=s, padding='same',
                           kernel_initializer=_kernel_init(),
                           kernel_regularizer=_regularizer(wd),
                           use_bias=False, name='conv'+str(unique_id_1))
        
        self.bn = BatchNormalization(name='bn'+str(unique_id_2))
...
class FPN(tf.keras.layers.Layer):
    ...
    def get_config(self):
        cfg = super().get_config().copy()
        cfg.update({'output1': self.output1,
...
class SSH(tf.keras.layers.Layer):
    ...
    def get_config(self):
        cfg = super().get_config().copy()
        cfg.update({'conv_3x3': self.conv_3x3,
